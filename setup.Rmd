...
<iframe width="560" height="315" src="https://www.youtube.com/embed/4B88dRbwNfc" frameborder="0" allowfullscreen></iframe>

We're not going to enter the client ID and client secret directly in any of these examples, but, rather, store them in the `.Renviron` file, which is detailed below. It's fine if you made the full bit of code as Donal described it in the video, but know that you will need to copy the `client_id` and `client_secret` into another file as detailed below.
...
This happens any time you start a new R session, so you won't have to worry about this going forward unless you change a value (like the *GA_EXAMPLE_VIEW_ID*). For this one time, though, select *Session > Restart R* in RStudio.
...
Some examples will use other packages, but that will be called out in the example itself. Almost all of the examples use the `googleAnalyticsR` and various packages in the Tidyverse, so run the code below in your console just to get those installed in the event that they are not:
